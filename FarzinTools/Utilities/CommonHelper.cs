...
namespace Ngra.Common.Utilities
{
    public static partial class CommonHelper
    {
        ...
        {
            //set minimum
            var resultTrustLevel = AspNetHostingPermissionLevel.None;
            ...
            })
            {
                try
                {
                    ...
                    //  We've set the highest permission we can
                    break;
                }
            ...
            return resultTrustLevel;
        }

        public static SqlParameter CreateSqlParameter(string name, object value, SqlDbType sqlDbType)
        {
            return new SqlParameter(name, sqlDbType) { Value = value ?? DBNull.Value };
        }

        public static SqlParameter CreateSqlParameter(string name, object value, SqlDbType sqlDbType, int size)
        {
            return new SqlParameter(name, sqlDbType, size) { Value = value ?? DBNull.Value };
        }

        public static SqlParameter CreateSqlParameter(string name, object value, SqlDbType sqlDbType, ParameterDirection direction)
        {
            return new SqlParameter(name, sqlDbType) { Value = value ?? DBNull.Value, Direction = direction };
        }

        public static SqlParameter CreateSqlParameter(string name, object value, SqlDbType sqlDbType, ParameterDirection direction, int size)
        {
            return new SqlParameter(name, sqlDbType, size) { Value = value ?? DBNull.Value, Direction = direction };
        }
        ...
        public static int CalculatePersianAge(int year, int month, int day)
        {
            var stringBirthday = year + "/" +
                                 month + "/" +
                                 day;

            DateTime christianBirthday = DateTimeManager.ToChristianDateTime(stringBirthday);

            var today = DateTime.Today;

            int age = today.Year - christianBirthday.Year;

            if (christianBirthday > today.AddYears(-age))
            {
                age--;
            }
        ...
        public static double GetLatitude(this string location)
        {
            if (string.IsNullOrWhiteSpace(location)) return 0;

            location = location.ToLower().Replace("point (", "").Replace(")", "");
        ...
        public static double GetLongitude(this string location)
        {
        ...
        public static double GetLatitude(DbGeometry location)
        {
            double latitude = 0;

            if (location == null) return latitude;

            var testLocation = location.AsText().ToLower().Replace("point (", "").Replace(")", "");
            latitude = double.Parse(testLocation.Split(' ')[0], new NumberFormatInfo { NegativeSign = "-" });

            return latitude;
        }

        public static double GetLongitude(DbGeometry location)
        {
            double longitude = 0;

            if (location == null) return longitude;

            var testLocation = location.AsText().ToLower().Replace("point (", "").Replace(")", "");

            longitude = double.Parse(testLocation.Split(' ')[1], new NumberFormatInfo { NegativeSign = "-" });
        ...
        public static DbGeometry ToPolygon(this string coordinates)
        {
            ...
            return DbGeometry.PolygonFromText(coordinates, 4326);
        }
        ...
        public static DateTime FindFirstPersianMonth(int persianYear, int persianMonth)
        {
            return new PersianCalendar().ToDateTime(persianYear, persianMonth, 1, 0, 0, 0, 0);
        }

        public static DateTime FindLastPersianMonth(int persianYear, int persianMonth)
        {
            if (persianMonth <= 11)
            {
                persianMonth++;
            ...
            var day = new PersianCalendar().ToDateTime(persianYear, persianMonth, 1, 0, 0, 0, 0);

            return day.AddDays(-1);
        }

        public static DateTime FindFirstPersianYear(int persianYear)
        {
            return FindFirstPersianMonth(persianYear, 1);
        }

        public static DateTime FindLastPersianYear(int persianYear)
        {
            return FindLastPersianMonth(persianYear, 12);
        }

        public static IEnumerable<PersianDateTime> GetLastPersianMonth(int count, int stepMonth = 0)
        {
            var result = new HashSet<PersianDateTime>();

            var persinaDate = DateTime.Now.ToPersianDateTime();

            for (var i = 0; i < count; i++)
            {
                var day = new PersianCalendar().ToDateTime(persinaDate.Year, persinaDate.Month, 1, 0, 0, 0, 0).AddMonths(stepMonth - i);

                result.Add(day.ToPersianDateTime());
            }
